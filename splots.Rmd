...
```{r ross_bio_mech}
tb <- table(av1x$ross_bio_mech)
print(tb)
f1 <- survfit(av_su~av1x$ross_bio_mech)
...
tb <- table(av1x$ross_bio_mech[av1x$ross_bio_mech!="999"])
print(tb)
f2 <- survfit(av_su~ross_bio_mech, data=av1x, subset=ross_bio_mech!="999")
end_points <- cumsum(f2$strata)
plot(f2, xlim=c(0, 48))
text(f2$time[end_points]+1, f2$surv[end_points], names(tb), adj=0)
title("Aortic valve replacement survival by replacement type")
```
